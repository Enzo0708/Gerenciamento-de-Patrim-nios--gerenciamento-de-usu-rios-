...
def buscar_salas(request):
    ...
    if is_coordenador:
        ...
        sala = Sala.objects.none()  # Usuário sem permissão não vê nada
...
def buscar_itens_sala(request):
    ...
    if is_coordenador:
        inventario = Inventario.objects.all()  # Coordenador vê todos os itens
    elif is_professor:
        # Professor vê apenas itens de suas salas
        inventario = Inventario.objects.filter(sala__in=Sala.objects.filter(responsavel=request.user.username).values_list('sala', flat=True))
    else:
        inventario = Inventario.objects.none()  # Usuário sem permissão não vê nada
...
def itens(request):
    ...
    if is_coordenador:
        ...
        # Professor vê apenas itens pelos quais é responsável
        inventario = Inventario.objects.filter(responsavel__responsavel=request.user.username)
    else:
        # Usuário sem permissão não vê nada
        inventario = Inventario.objects.none()
    ...
    # Renderiza a página 'itens.html' com o contexto adequado
